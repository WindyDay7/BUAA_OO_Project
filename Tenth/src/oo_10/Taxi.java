...
class Taxi {
	...
	*/
	...
	public void wander() {    //出租车处于游荡状态，四处游荡
		...
		tra_wan[4] = (this.location_y==79)? 10:guigv.GetFlow(location_y, location_x, location_y+1, location_x+1);
		...
		for(int i=1; i<=4; i++) {
			if(direction==1) {
				...
				if(this.location_x>0 && map_mess.graph[location][location_2]==1 && (tra_wan[1]<=tra_wan[wan_flag] || wan_flag==0)) {
					wan_tem_x = this.location_x-1;
					wan_tem_y = this.location_y;
					wan_flag = 2;
				}
				...
				if(this.location_y>0 && map_mess.graph[location][location_3]==1 && (tra_wan[1]<=tra_wan[wan_flag] || wan_flag==0)) {
				...
				if(this.location_y<79 && map_mess.graph[location][location_4]==1 && (tra_wan[1]<=tra_wan[wan_flag] || wan_flag==0)) {
					wan_tem_y = this.location_y+1;
					wan_tem_x = this.location_x;
					wan_flag = 4;
				}
