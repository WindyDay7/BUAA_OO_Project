...
class Super_Taxi extends Taxi {

	public Super_Taxi(int num, TaxiGUI taxi_gui, guiInfo map_in) {
		super(num, taxi_gui, map_in);
	}
	...
	public void move() {   //出租车运行，送往目的地
		...
		if(this.location_x==this.destination_x && this.location_y==this.destination_y) {
			...
			search_fps(destination,location);
			this.start_time += 500;
			String str4 = "车辆编号: " + this.number + "途经分支点的坐标: (" + this.location_x + "," + this.location_y + ") 经过时刻:" + this.start_time + "\r\n";				
	...
	public synchronized void search_fps(int destination, int location) {
		...
		GetPath(this.map_mess.D_D ,destination, this.map_mess.Old_Graph);
		length = map_mess.D_D[destination][location];
		...
		location_1 = (this.location_x==79)? location:(tem_y1*80 + tem_x1); //表示方向向东
		if(this.location_x<79) {
			length1 = map_mess.D_D[destination][location_1];
		...
		location_2 =(this.location_x==0)? location:(tem_y2*80 + tem_x2);  //表示方向向西
		if(this.location_x>0) {
			length2 = map_mess.D_D[destination][location_2];
		...
		location_3 =(this.location_y==79)? location:(tem_y3*80 + tem_x3);
		if(this.location_y<79) {
			length3 = map_mess.D_D[destination][location_3];
		...
		location_4 = (this.location_y==0)? location:(tem_y4*80 + tem_x4);
		if(this.location_y>0) {
			length4 = map_mess.D_D[destination][location_4];
			traffic[4] = map_mess.Traffic[destination][location_4] + this.map_mess.Flow[location_4][location];
		}
		
		//System.out.println(tem_x1 + "," + tem_y1 + "," + tem_x2 + "," + tem_y2 + "," + tem_x3 + "," + tem_y3 + "," + tem_x4 + "," + tem_y4);
		//判断下一个点的位置
		...
		//System.out.println("下一个点的坐标" + this.location_x + "," + this.location_y);
	}
	...
	public synchronized void GetPath(int Bfs_Map[][],int root2 , int Gra[][]) {
		//Requires: Bfs_Map，root2，Gra
		//Modifies: Bfs_Map
		//Effect：出租车的目的地root2 -> 计算Bfs_Map And Gra[][] And Traffic[][]，得到最短路劲
		int[] offset2 = new int[] { 0, 1, -1, 80, -80 };
		Vector<node> queue2 = new Vector<node>();
		boolean[] view2 = new boolean[6405];
		int x2 = root2;// 开始进行单点bfs

		for (int i = 0; i < 6400; i++) {
			for (int j = 0; j < 6400; j++) {
				if (i == j) {
					Bfs_Map[i][j] = 0;
					this.map_mess.Traffic[i][j] = 0;
					
				} else {
				    if(Bfs_Map[i][j]==0 || Bfs_Map[i][j]>7000) {
						Bfs_Map[i][j] = Gra[i][j];// 赋初值
					}
					this.map_mess.Traffic[i][j] = 0;	
				}
		...
		for (int i = 0; i < 6400; i++)
			view2[i] = false;
		queue2.add(new node(x2, 0));
		while (queue2.size() > 0) {
			node n2 = queue2.get(0);
			view2[n2.NO] = true;
			for (int i = 1; i <= 4; i++) {
				int next2 = n2.NO + offset2[i];
				if(next2 < 0 || next2 >= 6400) continue;
				if(Gra[n2.NO][next2] != 1) continue;    //不符合情况的道路直接pass，
				
				if (view2[next2] == false || (Bfs_Map[x2][next2]==Bfs_Map[x2][n2.NO]+1 && this.map_mess.Traffic[x2][next2]>this.map_mess.Traffic[next2][x2] + this.map_mess.Flow[n2.NO][next2])) {
					view2[next2] = true;
					queue2.add(new node(next2, n2.depth + 1));// 加入遍历队列
					this.map_mess.Traffic[x2][next2] = this.map_mess.Traffic[x2][n2.NO] + this.map_mess.Flow[n2.NO][next2];  //加上相邻两点之间的流量就可以了
					this.map_mess.Traffic[next2][x2] = this.map_mess.Traffic[x2][next2];
					Bfs_Map[x2][next2] = n2.depth + 1;
					Bfs_Map[next2][x2] = n2.depth + 1;
				}
			}
			queue2.remove(0);// 退出队列
		}
	}
