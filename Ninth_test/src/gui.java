...
class node {// 结点信息
    ...
    public node(int _NO, int _depth,int _flow) {
        // Requires:int类型的结点号,int类型的深度信息
        // Modifies:创建一个新的node对象，修改了此对象的NO,depth属性
        // Effects:创建了一个新的node对象并初始化
        NO = _NO;
...
//		}
		...
		while (queue.size() > 0) {
			...
			for (int i = 1; i <= 4; i++) {
				...
				if (next >= 0 && next < 6400 && view[next] == false && graph[n.NO][next] == 1) {
					view[next] = true;
					queue.add(new node(next, n.depth + 1,n.flow + Map.getflow(n.NO,next)));// 加入遍历队列
					D[x][next] = n.depth + 1;
					D[next][x] = n.depth + 1;
                    flow[x][next] = n.flow + Map.getflow(n.NO,next);
                    flow[next][x] = n.flow + Map.getflow(n.NO,next);
				}
			}
			queue.remove(0);// 退出队列
		}
    ...
    public int flow(int x1, int y1, int x2, int y2) {// 使用bfs计算两点之间的距离
        pointbfs(x1 * 80 + y1);
        return flow[x1 * 80 + y1][x2 * 80 + y2];
    }

    public void mypointbfs(int root,Point dst) {// 单点广度优先搜索
        // Requires:int类型的点号root
        // Modifies:D[][],System.out
        // Effects:对整个地图进行广度优先搜索，获得任意点到root之间的最短路信息，储存在D[][]中
        int[] offset = new int[] { 0, 1, -1, 80, -80 };
...
//		}
        int x = root,countaim=0;// 开始进行单点bfs
        for (int i = 0; i < 6400; i++)
            view[i] = false;
        queue.add(new node(x, 0));
        while (queue.size()>0) {
...
//                    flow[next][x] = n.flow + Map.getflow(n.NO,next);
//                }
                /*else*/ if (next >= 0 && next < 6400 && (D[x][next] > n.depth + 1) && graph[n.NO][next] == 1) {
                    view[next] = true;
                    queue.add(new node(next, n.depth + 1,n.flow + Map.getflow(n.NO,next)));// 加入遍历队列
                    D[x][next] = n.depth + 1;
                    if (flow[x][next]>n.flow + Map.getflow(n.NO,next)) {
                        flow[x][next] = n.flow + Map.getflow(n.NO, next);
                    }
                    if (next==aim){
                        countaim++;
...
class guigv {
	...
	public static boolean drawflow=true;//是否绘制流量信息
...
//      int flow = guigv.m.flow(src.x, src.y, dst.x, dst.y);
//		debugform form1 = new debugform();
//		form1.text1.setText("从(" + src.x + "," + src.y + ")到(" + dst.x + "," + dst.y + ")的最短路径长度是" + distance);
//      form1.text1.setText("从(" + src.x + "," + src.y + ")到(" + dst.x + "," + dst.y + ")的最小流量是" + flow);
	}
