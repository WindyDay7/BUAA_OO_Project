	...
	public void pointbfs(int root) {// 单点广度优先搜索
		...
		while (queue.size() > 0) {
			...
			for (int i = 1; i <= 4; i++) {
				...
				if(next < 0 || next >= 6400) continue;
				if(graph[n.NO][next] != 1) continue;    //不符合情况的道路直接pass，
				int x1=n.NO/80, y1=n.NO%80, x2=next/80, y2=next%80;   //计算出两点的坐标，相邻两点
				if (view[next] == false || (D[x][next]==D[x][n.NO]+1 && traffic[x][next]>traffic[next][x] + guigv.GetFlow(x1,y1,x2,y2))) {
					view[next] = true;
					queue.add(new node(next, n.depth + 1));// 加入遍历队列
					traffic[x][next] = traffic[x][n.NO] + guigv.GetFlow(x1,y1,x2,y2);  //加上相邻两点之间的流量就可以了
					traffic[next][x] = traffic[x][next];
					D[x][next] = n.depth + 1;
					D[next][x] = n.depth + 1;
				}
			}
...
class guigv {
	...
	public static void AddFlow(int x1,int y1,int x2,int y2){//增加一个道路流量
		synchronized (guigv.flowmap) {
			...
			//添加流量
			guigv.flowmap.put(Key(x1,y1,x2,y2), 1);
			guigv.flowmap.put(Key(x2,y2,x1,y1), 1);
			System.out.println("流量明明就加了啊" + guigv.GetFlow(x1,y1,x2,y2));
		}
	}
	...
	public static void ClearFlow(int x1,int y1,int x2,int y2){//清空流量信息
		synchronized (guigv.flowmap) {
			int count = 0;
			count = guigv.flowmap.get(Key(x1, y1, x2, y2)) == null ? 0 : guigv.flowmap.get(Key(x1, y1, x2, y2));
			// 添加流量
			guigv.flowmap.put(Key(x1, y1, x2, y2), count - count);
			guigv.flowmap.put(Key(x2, y2, x1, y1), count - count);
		}
	}
}
