...
class Override extends Scheduler{
	...
	protected int count_num = 0; //表示有效指令的数目
	...
	protected void deal_t() {
		...
		int k = i;
		//至此表示生成出主请求结束
		if(k==this.i)  //表示上一条执行的指令就是主请求，及主请求被执行
		{   //表示将请求队列中的请求加入捎带
			while((k < this.count_num) && (this.request_n[k][1] <= j)) //
			{
				arrive = 0;
	...
	/*4*/		else if((this.request_n[k][0]<this.station[0] && this.request_n[k][0]>=this.pr_instr[0]) && this.pr_instr[0]<=this.station[0] && this.request_n[k][3]==2 && this.request_n[k][4]==0) {
					...
					if(this.pr_instr[0]==this.request_n[k][0] && this.pr_instr[2]==this.request_n[k][2] && this.pr_instr[3]==this.request_n[k][3] &&  this.request_n[k][4]==0)
					{
						this.request_n[k][4] = 1;
						System.out.printf("#SAME");
						output2(this.request_n[k]);
