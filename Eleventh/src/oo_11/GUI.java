...
class guiInfo {
	...
	int[][] Old_Graph = new int[6405][6405];
	int[][] D = new int[6405][6405];// 保存从i到j的最小路径值
	int[][] Traffic = new int[6405][6405];
	int[][] D_D = new int[6405][6405];
	int[][] Flow = new int[6405][6405];
		...
		for (int i = 0; i < 80; i++) {
			for (int j = 0; j < 80; j++) {
				if (map[i][j] == 1 || map[i][j] == 3) {
					...
					this.Old_Graph[i * 80 + j][i * 80 + j + 1] = 1;
					this.Old_Graph[i * 80 + j + 1][i * 80 + j] = 1;
				}
				if (map[i][j] == 2 || map[i][j] == 3) {
					graph[i * 80 + j][(i + 1) * 80 + j] = 1;
					graph[(i + 1) * 80 + j][i * 80 + j] = 1;
					this.Old_Graph[i * 80 + j][(i + 1) * 80 + j] = 1;
					this.Old_Graph[(i + 1) * 80 + j][i * 80 + j] = 1;
				}
			}
		}
	}
	...
	public synchronized void pointbfs(int root) {// 单点广度优先搜索
		...
		while (queue.size() > 0) {
			...
			for (int i = 1; i <= 4; i++) {
				...
				if (view[next] == false || (D[x][next]==D[x][n.NO]+1 && Traffic[x][next]>Traffic[next][x] + Flow[n.NO][next])) {
					view[next] = true;
					queue.add(new node(next, n.depth + 1));// 加入遍历队列
					Traffic[x][next] = Traffic[x][n.NO] + Flow[n.NO][next];  //加上相邻两点之间的流量就可以了
...
class guigv {
	...
	/* GUI */
	public static int num_MAX = 100000;
	public static int Light_time = 0;
	public static long Light_Still = 0;
